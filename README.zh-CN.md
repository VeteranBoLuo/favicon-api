...
返回有效且成功的图标获取次数，不统计体验页自动加载的示例预览。

```json
{ "count": 12001 }
```

公开数字对“同一来源 + 规范化后的同一域名”每 30 分钟最多计一次；不同域名仍会正常计数，因此不会排除正常的批量书签导入。单一来源在 10 分钟内超过 3000 个不同域名后，只暂停增加公开数字，API 仍正常响应。仅当反向代理会覆盖 `X-Real-IP` 时才设置 `FAVICON_TRUST_PROXY=1`；可信的第一方批量导入来源可通过 `FAVICON_USAGE_TRUSTED_IPS` 绕过极端阈值。
