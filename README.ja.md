...
有効かつ成功した favicon 取得数を返します。体験ページが自動表示するサンプルは集計しません。

```json
{ "count": 12001 }
```

同一クライアントと正規化済みホスト名の組み合わせは 30 分に 1 回だけ集計します。異なるホスト名は通常どおり加算されるため、正当な一括インポートは除外されません。10 分間に 3000 個を超える異なるホスト名へ到達した場合も、停止するのは公開カウンターだけで、API は通常どおり応答します。`FAVICON_TRUST_PROXY=1` は `X-Real-IP` を上書きするリバースプロキシ配下でのみ使用してください。
