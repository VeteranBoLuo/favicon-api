...
유효하고 성공한 favicon 가져오기 수를 반환합니다. 체험 페이지에서 자동으로 불러오는 예시 미리보기는 집계하지 않습니다.

```json
{ "count": 12001 }
```

동일한 클라이언트와 정규화된 호스트 이름 조합은 30분에 한 번만 집계합니다. 서로 다른 호스트 이름은 계속 집계되므로 정상적인 대량 북마크 가져오기는 제외되지 않습니다. 한 클라이언트가 10분 안에 3000개가 넘는 서로 다른 호스트 이름을 요청해도 공개 카운터만 멈추며 API 응답은 계속 정상 제공됩니다. `FAVICON_TRUST_PROXY=1`은 `X-Real-IP`를 덮어쓰는 리버스 프록시 뒤에서만 사용해야 합니다.
...
# 사용 횟수를 재시작 후에도 유지하고 기존 실제 수치를 초기값으로 설정
